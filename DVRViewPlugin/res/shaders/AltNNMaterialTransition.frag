...
void processVoxel(inout int i, inout vec3 samplePos, inout float lengthRay, inout vec3 voxelCoord, inout float currentMaterial, inout float[5] materials, inout float[5] voxelRayLengths, vec3 rayDir, vec3 rayOffset, vec3 dimensions, inout vec3 tNext, vec3 tDelta, inout float t) {
...
void main() {
    ...
    vec3 rayOffset = (rayDir * 0.0001) / dimensions;
    ...
    while (lengthRay > 0) {
        i = (i - 1);
        processVoxel(i, samplePos, lengthRay, voxelCoord, currentMaterial, materials, voxelRayLengths, rayDir, rayOffset, dimensions, tNext, tDelta, t);
        if (lengthRay <= 0.0){
            break;
        }
